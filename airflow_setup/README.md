...
│  │  Webserver :8080 / Scheduler               │  ├─ airflow DB  (메타데이터)
│  │    │ DAG 실행 스케줄                  ◄──►  │  └─ stock_data DB (수집 데이터)
│  │    ▼                                       └─────────────────────────
│  │  Yahoo Finance API  ─── 데이터 저장 ──────────────────────────►
│  │  Google News RSS    ─── 데이터 저장 ──────────────────────────►
│  └─────────────────────────────────────────
│
│  ┌─ Streamlit :8501 ──────────────────────
│  │  stock_data DB 조회
...
│  │  Python 3.11   │  │  Postgres 15   │  │  Python 3.11   │
│  │  Port: 8080    │  │  Port: 5432    │  │  Port: 8501    │
│  └────────────────┘  └────────────────┘  └────────────────┘
│
...
┌─ yfinance_news_group  (상장 18개 종목 병렬) ────────────────────────────
│  ticker.news API 호출
└──────────────────────────────────────────────────────────────────────────

┌─ google_news_group  (20개 쿼리: 비상장사 포함) ─────────────────────────
│  Google News RSS 피드 파싱 (feedparser 라이브러리)
└──────────────────────────────────────────────────────────────────────────
  ...
  · 비상장사(TerraPower, X-Energy)는 google_news_group만 사용
```
